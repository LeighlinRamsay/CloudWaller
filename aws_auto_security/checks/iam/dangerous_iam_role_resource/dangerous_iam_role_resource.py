#!/usr/bin/env python3
"""
Plugin ID: dangerous_iam_role_resource
Description: IAM roles whose policies allow wildcard resources (“*”).
"""
...
class Plugin:
    ...
    def run(self):
        """
        Scan all IAM roles for any policy (inline or attached) allowing Resource="*" .
                ...
                for ip_page in ip_pag.paginate(RoleName=role):
                    for pname in ip_page.get('PolicyNames', []):
                        try:
                            ...
                            for s in stmts:
                                res = s.get('Resource')
                                if s.get('Effect')=='Allow' and (res=='*' or (isinstance(res, list) and '*' in res)):
                                    findings.append((role, 'Role allows wildcard (*) resource scope'))
                                    break
                        except ClientError:
                            continue
                # attached managed
